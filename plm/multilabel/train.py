...
def main(config):
    ...
    train_df = pd.read_csv(config['data']['train']).fillna('None')
    valid_df = pd.read_csv(config['data']['valid']).fillna('None')

    tokenized_train_dataset = Dataset.from_pandas(train_df[[ 'prompt', 'A', 'B', 'C', 'D', 'E', 'answer']]).map(preprocess, remove_columns=[ 'prompt', 'A', 'B', 'C', 'D', 'E', 'answer'])
    data_collator = DataCollatorForMultipleChoice(tokenizer=tokenizer)
    train_dataloader = DataLoader(tokenized_train_dataset, batch_size=4, shuffle=True, collate_fn=data_collator)
